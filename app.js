...
const map=L.map("map",{zoomControl:false,zoomSnap:0.5,minZoom:5,maxZoom:12,maxBounds:UKRAINE_BOUNDS,maxBoundsViscosity:1}).setView(UKRAINE_CENTER,6);
...
function show(e){
 card.innerHTML=`<h2>${e.region}</h2><span class="badge">${e.status}</span><p>${e.text}</p><div class="source"><b>Источник:</b> <a href="${e.url}" target="_blank" rel="noopener">${e.source}</a></div>`;
}
function render(){
 ...
 list.forEach(e=>{
   const m=L.marker([e.lat,e.lon],{icon:icon(e)}).addTo(map);
...
function fixMapSize(){
 setTimeout(()=>{map.invalidateSize(false);map.setView(UKRAINE_CENTER,6,false)},100);
 setTimeout(()=>map.invalidateSize(false),700);
 setTimeout(()=>map.invalidateSize(false),1800);
}
render();
fixMapSize();
