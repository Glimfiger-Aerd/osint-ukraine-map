...
def main():
    ...
    # --- ИСПРАВЛЕННЫЙ БЛОК TELEGRAM ---
    for name, handle in TG_CHANNELS:
        try:
            tg_page = get(f"https://t.me/s/{handle}")
            
            # Ультимативный метод: режем страницу по атрибуту, который Telegram присваивает каждому посту
            posts = tg_page.split('data-post="')[1:]
            
            for post_html in posts:
                # Ищем текст
                text_m = re.search(r'<div class="tgme_widget_message_text[^>]*>(.*?)</div>', post_html, re.S | re.I)
                if not text_m: continue
                text = clean(text_m.group(1))
                
                # Ищем ссылку и картинку
                link_m = re.search(r'<a class="tgme_widget_message_date" href="(https://t\.me/[^/]+/\d+)">', post_html, re.I)
                img_m = re.search(r"background-image:\s*url\(['\"]?([^'\")]+)['\"]?\)", post_html, re.I)
